...
class CurrentUserServiceImplTest {
    ...
    void hasResourceAuthority_shouldReturnTrue_whenUserHasResourceAuthority() {
        ...
        // When
        var result = currentUserService.hasResourceAuthority(resourceUniqueId, permission);
    ...
    @Test
    void hasResourceAuthority_shouldReturnTrue_whenUserIsIdentityAdmin() {
        // Given
        var userDetails = mock(UserDetails.class);
        ...
        when(authentication.isAuthenticated()).thenReturn(true);
        when(authentication.getPrincipal()).thenReturn(userDetails);
        when(userDetails.getAuthorities()).thenAnswer(invocation -> List.of(new SimpleGrantedAuthority("ROLE_IDENTITY_ADMIN")));

        // When
        var result = currentUserService.hasResourceAuthority(123L, "read");
